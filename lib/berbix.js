...
Client.prototype.uploadImages = async function (tokens, uploadImageOpts) {
  ...
  try {
    const resp = await this._tokenAuthRequest("POST", tokens, true, "/v0/images/upload", payload);
    return adaptImageUploadResponse(resp)
  } catch (e) {
...
// Convert an ImageUploadResponse from the API into an object with camel-cased
function adaptImageUploadResponse(uploadRes) {
  let result = {
