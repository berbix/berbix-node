...
Client.prototype.createHostedTransaction = async function (opts) {
  var payload = optionsToPayloadObject(opts);
  const txnResp = await this._createTransaction("/v0/transactions", payload);
  return {
    tokens: txnResp.tokens,
    hosted_url: txnResp.hostedProperties["hosted_url"],
  };
};
