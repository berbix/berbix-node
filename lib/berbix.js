...
Client.prototype._request = function(method, path, headers, data) {
  var base = url.parse(this.apiHost);
...
Client.prototype._refreshIfNecessary = function(userTokens) {
  if (userTokens.needsRefresh()) {
    var refreshed = this.refreshTokens(userTokens);
    userTokens.refresh(refreshed.accessToken, refreshed.expiry);
  }
};

Client.prototype._tokenAuthRequest = function(method, userTokens, path) {
  this._refreshIfNecessary(userTokens);
  var headers = {
