...
function Tokens(refreshToken, accessToken, clientToken, expiry, transactionId) {
...
Tokens.prototype.refresh = function(accessToken, clientToken, expiry, transactionId) {
  this.accessToken = accessToken;
  this.clientToken = clientToken;
  this.expiry = expiry;
  this.transactionId = transactionId;
...
Client.prototype._fetchTokens = async function(path, payload) {
  ...
  return new Tokens(result.refresh_token, result.access_token, result.client_token, now() + result.expires_in, result.transaction_id);
};
...
Client.prototype.createContinuation = async function(tokens) {
  var result = await this._tokenAuthRequest('POST', tokens, '/v0/continuations');
  return result.value;
};
