...
function UserTokens(refreshToken, accessToken, expiry, userId) {
...
UserTokens.prototype.refresh = function(accessToken, expiry, userId) {
  this.accessToken = accessToken;
  this.expiry = expiry;
  this.userId = userId;
};
...
Client.prototype._fetchTokens = async function(path, payload) {
  ...
  return new UserTokens(result.refresh_token, result.access_token, now() + result.expires_in, result.user_id);
};
...
Client.prototype._refreshIfNecessary = async function(userTokens) {
  if (userTokens.needsRefresh()) {
    ...
    userTokens.refresh(refreshed.accessToken, refreshed.expiry, refreshed.userId);
  }
};
